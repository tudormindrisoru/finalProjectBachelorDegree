...
class PhoneAuth {
  ...
  async insertIntoDB() {
    try {
      ...
      return new Response(500, false, "Eroare de server!");
    } catch (err) {
      console.error(err);
  ...
  async removeFromDB() {
    try {
      ...
      if (result) {
        return new Response(200, true, "Entitate stearsa.");
      }
      return new Response(204, false, "Entitatea a fost stearsa deja!");
    } catch (err) {
      console.error(err);
      return new Response(500, false, err);
    }

...
router.post('/login-with-password', async(req, res) => {
    ...
    if(error) {
        res.status(400).send({ "result": new Response(400, false, error.details[0].message) });
    } else {
...
router.post('/login-with-phone-step1', async(req, res) => {
    ...
    if(error) {
        ...
        if(response.status !== 200) {
            ...
            res.status(201).send(new Response(201, true, "Code generated! please send the code received via SMS!").getResponse());
        }
    }
});
        ...
        if(!result) {
            ...
            if(result.code === req.body.code) {
                ...
                if(user) {
                    ...
                    res.status(404).send(new Response(404, false, "User not found!").getResponse());
                }
            } else {
                res.status(403).send(new Response(403, false, "The code is not correct!").getResponse());
            }
        }
    }
});
...
router.post('/register-step1', async(req, res) => {
    ...
    if(error) {
        ...
        if(validator.validate(req.body.email)) {
            ...
            const user = new User(req.body.firstName, req.body.lastName, req.body.email, req.body.password, req.body.phone);
            ...
            res.status(400).send(new Response(400, false, "This email is invalid.").getResponse());
        }
    }
});
    ...
    if(error) {
        ...
        if(!result) {
            ...
            if(result.code === req.body.code) {
                ...
                const user = new User(undefined, undefined, undefined, undefined, req.body.phone);
                console.log(user);
